...
class gl_AddDialog(Gtk.Dialog):
    ...
    def on_entry_lostfocus(self, widget, event, entry_index):
        ...
        try:
            ...
            dialog = Gtk.MessageDialog(self,0,Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, "TYPE ENTRY ERROR")
            dialog.format_secondary_text("The data for this entry must be of type %s" % self.type_dict[entry_index])
            dialog.run()
            dialog.destroy()
